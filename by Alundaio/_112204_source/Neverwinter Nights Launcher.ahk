...
; Added newer Update-Url
UPDATE_URL : = "http://neverwintervault.org/"
;===============================
...
Loop, 5
{
  ...
  if (Server != "" && ServerIP%A_Index% !="")
  {
    ...
    Gui, 1: Add, picture, C enter x82 hwnd%Server%_hwnd gServerLaunchURL%A_Index%, %A_WorkingDir%\DCLdata\Banners\%Banner%
...
;Added support for NWNCX
  if (%EnableNWNCX% == 0)
  {
    Gui, 2: Add, CheckBox, x22 y52 w130 h30 vEnableNWNCX, Enable using NWN client extender
  }
  else
  {
    Gui, 2: Add, CheckBox, x22 y52 w130 h30 vEnableNWNCX checked, Enable using NWN client extender
  }

; Added support for NWNTX
  if (%EnableNWNTX% == 0)
  {
    Gui, 2: Add, CheckBox, x22 y52 w130 h30 vEnableNWNTX, Enable using NWN toolset extender
  }
  else
  {
    Gui, 2: Add, CheckBox, x22 y52 w130 h30 vEnableNWNTX checked, Enable using NWN toolset extender
  }
...
ShowAbout:
  ...
  HLink_Add(hGui3, 45,  130,  250, 20, "OnLink", "Thank you. Please remember to 'vote':neverwintervault.org !" )

  Gui, 3: Show, x373 y196 h272 w339, About
Return

LaunchVote:
  ; New address for new vault added
  run http://neverwintervault.org/project/nwn1/other/tool/nwn-1-direct-connect-launcher-alundaio-v-111-now-sources
Return
  ...
  if exist, update.ini
  {
    ...
    if (UPDATE_VERSION > CURRENT_VERSION)
    {
      ...
      if MsgBox Yes
      {
        ...
        {
          MsgBox, Failed. Download manually at http://neverwintervault.org/project/nwn1/other/tool/nwn-1-direct-connect-launcher-alundaio-v-111-now-sources
        }
      }
    }
    else
...
ToolsetLaunch:
  ...
  if (PersistantLauncher == 0)
  {
    run, NWNTX Launcher.exe, %NWN_DIR%, , NewPID
  }
  else
  {
...
CEPUpdater:
; Updated CEP Updater due outdated systems
  run http://neverwintervault.org/cep
...
2GuiSubmit:
  ...
  iniWrite, %ServerBanner2%,D CLconfig.ini, Server 2, Banner
