...
ReadLanguage()
{
  Global
    ...
    IniRead, Server1BannerLinkText, language.ini, %IniLangChoosen%, Server1BannerLinkText, Server 1
    IniRead, Server2BannerLinkText, language.ini, %IniLangChoosen%, Server2BannerLinkText, Server 2
    IniRead, Server3BannerLinkText, language.ini, %IniLangChoosen%, Server3BannerLinkText, Server 3
    IniRead, Server4BannerLinkText, language.ini, %IniLangChoosen%, Server4BannerLinkText, Server 4
    IniRead, Server5BannerLinkText, language.ini, %IniLangChoosen%, Server5BannerLinkText, Server 5
    ...
    IniRead, MsgBoxUpdateAvailable, language.ini, %IniLangChoosen%, MsgBoxUpdateAvailable, There is an update available! Would you like to download it? `n`nPlease remember to vote. Thanks!
    ...
    IniRead, MsgBoxStartNWNcX, language.ini, %IniLangChoosen%, MsgBoxStartNWNcX, Loading NWNCX... Stay tuned!`n`nRemember to start NWN DC Launcher`nwith admin-rigths
...
;counts the letters in the haystack to the beginning of the needle
GetTokenPosition(haystack, needle, delim="`|")
{
  ;ENGLISH|GERMAN|
  ;counted := GetTokenCount(haystack)  ;ex. with the haystack like ENGLISH|GERMAN| it would be 2
  ;Position := InStr(Haystack, Needle [, CaseSensitive?, StartingPos]) ;See the InStr() function for details.
  ;position := InStr(haystack, needle) ;ex. GERMAN as needle would be 9
  ;OutputVar := StrLen(InputVar)
  ;length := StrLen(haystack)          ;ex. with a haystack like ENGLISH|GERMAN| it would be 15
  ...
  Loop, Parse, haystack, %delim%
  {
    ...
    {
      result := position ; Nimm eins weg, da es alle Tokens nach dem letzten Delim zaehlt
    }
  }
  return result
}
