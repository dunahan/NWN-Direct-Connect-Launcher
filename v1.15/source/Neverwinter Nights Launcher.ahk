...
; As suggested by Vanar on neverwintervault.org, added more serveroptions
;===============================
...
Loop, 10
{
  ...
  If (Server != "" && ServerIP%A_Index% !="")
  {
    ...
    Gui, 1: Add, Picture, Center x82 w350 h-1 gServerLaunchURL%A_Index%, %A_WorkingDir%\DCLdata\Banners\%Banner%
...
;Ten Server URL's for all Servers
ServerLaunchURL1:
  Run, %ServerURL1%
Return
...
ServerOptions:
{
  ...
  ;Main GUI disabling and building Config GUI
  Gui, +disabled
  Gui, 2: +owner1
  Gui, 2: +LastFound
  ...
  Gui, 2: Add, Button, x272 y392 w100 h30 g2GuiSubmit, Ok
  Gui, 2: Add, Button, x422 y392 w100 h30 g2GuiClose, Cancel

  Gui, 2: Add, Tab, x2 y2 w760 h430, %ServerSettingsTabText%|%BannerOptionsTabText%|%ConfigurationTabText%|%GameOptionsTabText%

  Gui, 2: Tab, %ServerSettingsTabText%
  Gui, 2: Add, GroupBox, x12  y52 w140 h330, %ServerNameText%
  Gui, 2: Add, GroupBox, x162 y52 w140 h330, %ServerIPaddressText%
  Gui, 2: Add, GroupBox, x312 y52 w140 h330, %ServerPasswordText%
  Gui, 2: Add, GroupBox, x462 y52 w140 h330, %DMPasswordText%
  Gui, 2: Add, GroupBox, x612 y52 w140 h330, %BannerText%

  ;Add Server controls for all ten servers
  Loop, 10
  {
  ...
  Gui, 2: Add, GroupBox, x202 y32 w380 h330, %BannerOnClickURLText%
  Gui, 2: Add, Text, x222 y62, %ServerBannerLinkText% 1:
  Gui, 2: Add, Text, x222 y92, %ServerBannerLinkText% 2:
  Gui, 2: Add, Text, x222 y122, %ServerBannerLinkText% 3:
  Gui, 2: Add, Text, x222 y152, %ServerBannerLinkText% 4:
  Gui, 2: Add, Text, x222 y182, %ServerBannerLinkText% 5:
  Gui, 2: Add, Text, x222 y212, %ServerBannerLinkText% 6:
  Gui, 2: Add, Text, x222 y242, %ServerBannerLinkText% 7:
  Gui, 2: Add, Text, x222 y272, %ServerBannerLinkText% 8:
  Gui, 2: Add, Text, x222 y302, %ServerBannerLinkText% 9:
  Gui, 2: Add, Text, x222 y332, %ServerBannerLinkText% 10:

  Gui, 2: Add, Edit, x282 y62 w270 h20 vServerURL1, %ServerURL1%
  Gui, 2: Add, Edit, x282 y92 w270 h20 vServerURL2, %ServerURL2%
  Gui, 2: Add, Edit, x282 y122 w270 h20 vServerURL3, %ServerURL3%
  Gui, 2: Add, Edit, x282 y152 w270 h20 vServerURL4, %ServerURL4%
  Gui, 2: Add, Edit, x282 y182 w270 h20 vServerURL5, %ServerURL5%
  Gui, 2: Add, Edit, x282 y212 w270 h20 vServerURL6, %ServerURL6%
  Gui, 2: Add, Edit, x282 y242 w270 h20 vServerURL7, %ServerURL7%
  Gui, 2: Add, Edit, x282 y272 w270 h20 vServerURL8, %ServerURL8%
  Gui, 2: Add, Edit, x282 y302 w270 h20 vServerURL9, %ServerURL9%
  Gui, 2: Add, Edit, x282 y332 w270 h20 vServerURL10, %ServerURL10%

  Gui, 2: Add, Button, x635 y52 w80 h20 gOpenBannersFolder, %BannersButtonText%
  
  Gui, 2: Tab, %ConfigurationTabText%
  Gui, 2: Add, GroupBox, x12 y32 w220 h275, %OptionsText%
  If (%EnableLogging% == 0)
  {
    Gui, 2: Add, CheckBox, x22 y52 w130 h30 vEnableLogging, %EnableLogRenamingLblText%
    Gui, 2: Add, Edit, x22 y82 w130 h20 vOtherLogRotator, %OtherLogRotator% ;user can use other prog
  }
  Else
  {
    Gui, 2: Add, CheckBox, x22 y52 w130 h30 vEnableLogging checked, %EnableLogRenamingLblText%
    Gui, 2: Add, Edit, x22 y82 w130 h20 vOtherLogRotator ReadOnly, %OtherLogRotator% ;stop user to use others
  }
  Gui, 2: Add, Button, x162 y45 w60 h20 gOpenLogFolder, %LogsButtonText%
  Gui, 2: Add, Button, x162 y65 w60 h20 gOtherLogProg, %BrowseButtonText%
  Gui, 2: Add, Button, x162 y85 w60 h20 gConfigOtherLogProg, %ConfigRunText%
  
  If (%PersistantLauncher% == 0)
  {
    Gui, 2: Add, CheckBox, x22 y112 w130 h30 vPersistantLauncher, %PersistantLauncherLblText%
  }
  Else
  {
    Gui, 2: Add, CheckBox, x22 y112 w130 h30 vPersistantLauncher checked, %PersistantLauncherLblText%
  }
  
  If (%NWNCxLauncher% == 0)
  {
    Gui, 2: Add, CheckBox, x22 y142 w130 h30 vNWNCxLauncher, %NWNCXLauncherCBText%
  }
  Else
  {
    Gui, 2: Add, CheckBox, x22 y142 w130 h30 vNWNCxLauncher checked, %NWNCXLauncherCBText%
  }
  
  Gui, 2: Add, Text, x22 y175, %NWNxCFileNameText%:
  Gui, 2: Add, Edit, x92 y172 w130 h20 vNWNCxFile, %NWNCxFile% ;if needed, user can specify a other file-name

  If (%NWNTxLauncher% == 0)
  {
    Gui, 2: Add, CheckBox, x22 y202 w200 h30 vNWNTxLauncher, %NWNTXLauncherCBText%
  }
  Else
  {
    Gui, 2: Add, CheckBox, x22 y202 w200 h30 vNWNTxLauncher checked, %NWNTXLauncherCBText%
  }
  
  Gui, 2: Add, Text, x22 y235, %NWNxTFileNameText%:
  Gui, 2: Add, Edit, x92 y232 w130 h20 vNWNTxFile, %NWNTxFile% ;if needed, user can specify a other file-name
  
  ;; Languages
  LangPos := GetTokenPosition(DCLanguagesOptions, DCLanguagesChoosen) ; Get position of choosen language
  Gui, 2: Add, Text, x22 y262, %DCLanguagesText%:
  Gui, 2: Add, DDL, vChangeLangVar Choose%LangPos%, %DCLanguagesOptions% ; Display choosen lang in DropDownList
...
; remark!
  Gui, 2: Show, xCenter yCenter h438 w769, %GuiWindowNameText% ;Server Options ;is it possible to translate???
...
ShowAbout:
  ...
  Gui, 3: Add, Text, x12 y12, Neverwinter Nights DC Launcher v%CURRENT_VERSION% `n`nOriginal Author: Alundaio (with all his work!)`n`nUpdated Content: Dunahan`n`nWhatsNew:`n- Enable to Config/Run other Log-Rotators from within DC Launcher`n- Some more Bugfixes`n`nThank you for using!
  ...
  Gui, 3: Show, x373 y196 h272 w339, %ShowAboutText%
Return
...
ConfigOtherLogProg:
  If (%EnableLogging% == 0)
  {
    If (OtherLogRotator == "" or OtherLogRotator == "-")
      MsgBox, %MsgFailedOtherLogRotator%
    Else
      SplitPath, OtherLogRotator, OutFileName, OutDir, OutExtension, OutNameNoExt, OutDrive
      Run, %OutFileName%, %OutDir%, , NewPID
  }
  Else
  {
    GoSub, ShowLogRenamingConfig
  }
Return

ShowLogRenamingConfig:
  Gui, 2: +disabled
  Gui, 4: +owner1
  Gui, 4: +LastFound
  hGui4 := winexist()
  Gui, 4: Add, Button, x122 y222 w100 h30 g4GuiClose , Ok
  Gui, 4: Add, Text, x12 y12, Not available yet. Stay tuned ;-)
  Gui, 4: Show, x373 y196 h272 w339, %LogRotatingConfig%
Return
...
OtherLogProg:
  FileSelectFile, SelectedFile, 3, %A_WorkingDir%, Open a file, Executeable (*.exe)
  If SelectedFile =
  MsgBox, %MsgBoxBrowseFile%
  Else
  {
    GuiControl, , OtherLogRotator, %SelectedFile%
    OtherLogRotator = %SelectedFile%
  }
Return
...
2GuiSubmit:
  ...
  IniWrite, %OtherLogRotator%, DCLconfig.ini, Server Options, OtherLogRotator
  ...
  Loop, 10  ;Servers
  {
    IDS := ServerName%A_Index%
    IniWrite, %IDS%, DCLconfig.ini, Server %A_Index%, Name
    IDS := ServerIP%A_Index%
    IniWrite, %IDS%, DCLconfig.ini, Server %A_Index%, IPAddress
    IDS := ServerPassword%A_Index%
    IniWrite, %IDS%, DCLconfig.ini, Server %A_Index%, Password
    IDS := ServerDMPassword%A_Index%
    IniWrite, %IDS%, DCLconfig.ini, Server %A_Index%, DMPassword
    IDS := ServerBanner%A_Index%
    IniWrite, %IDS%, DCLconfig.ini, Server %A_Index%, Banner
    IDS := ServerURL%A_Index%
    IniWrite, %IDS%, DCLconfig.ini, Server %A_Index%, ServerURL
  }
