...
ServerOptions:
{
  ...
  Gui, 2: Add, Tab, x2 y2 w760 h310, %OptionsTabsText% ;Server Settings|Banner Options|Configuration|Game Options ;Original name of the field
  ...
  ;; Languages in work on progress, temporary disabled
  ;Gui, 2: Add, Text, x22 y232, %DCLanguagesText%:
  ;Gui, 2: Add, DDL, vDCLanguagesChoosen, %DCLanguagesOptions%
...
ShowAbout:
  ...
  Gui, 3: Add, Text, x12 y12, Neverwinter Nights DC Launcher v %CURRENT_VERSION% `n`nOriginal Author: Alundaio (with all his work!)`n`nUpdated Content: Dunahan`n`nWhatsNew:`n- Added languages`n- Some Bugfixes`n`nThank you for using!
  Gui, 3: Add, Link,, <a href="https://neverwintervault.org/project/nwn1/other/tool/nwn-1-direct-connect-launcher">Please vote here for this version</a>AND
...
SwapTlk:
	...
	If (TlkSwapFirstTime == 1)
	{
		IfExist, %TLK_DEFAULT%
		{
			...
			If ErrorLevel
				MsgBox, %MsgBoxRenameTlkFail% ;%TLK_DIR% could not be renamed to %TLK_DEFAULT%. ;is it possible to translate???
			Else
			{
				MsgBox, %MsgBoxSwapSuccess%			
...
2GuiSubmit:
  ...
  IniWrite, %NWNCxFile%, DCLconfig.ini, NWNCxFile, NWNCxFile
  IniWrite, %NWNTxFile%, DCLconfig.ini, NWNTxFile, NWNTxFile
  
  ;Languages Work in Progress!
  ;IniWrite, %DCLanguagesOptions%, language.ini, CHOOSEN, DCLanguagesOptions
  ;IniWrite, %DCLanguagesChoosen%, language.ini, CHOOSEN, DCLanguagesChoosen
